...
export const getIndexList = async (endpoint, offset) => {
  const data = await client.getList({
    endpoint: endpoint,
    queries: {
      offset: offset,
      limit: process.env.PAGING,
...
//CSRこっちじゃないとページネーションが反応しない
export const getItemList = async function(index){
  try {
    //console.log(("||MSG-getLists||query: " + JSON.stringify(index));
    //console.log(("||MSG getItems|| " + index.offset);
      const query = { limit: 2, offset: index.offset*process.env.NEXT_PUBLIC_PAGING };
